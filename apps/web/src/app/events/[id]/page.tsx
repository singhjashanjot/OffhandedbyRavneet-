...
/* ========================================
   EVENT DETAIL PAGE
   Fetches workshop from Supabase by ID
======================================== */

// Dynamic page — no static params needed for DB-driven data
...
export default async function EventDetailsPage({
  ...
  // Get related workshops (same category, exclude current)
  const { data: relatedWorkshops } = await supabase
    .from("workshops")
    .select("*")
    .neq("id", workshop.id)
    .eq("is_active", true)
                  ...
                  <div className="space-y-4 mb-8">
                    <div className="flex items-start gap-4 p-3 rounded-lg hover:bg-neutral-50 transition-colors">
                      ...
                      <div>
                        ...
                        {timeDisplay && <p className="text-sm text-neutral-500">{timeDisplay}</p>}
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-4 p-3 rounded-lg hover:bg-neutral-50 transition-colors">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-neutral-500">{workshop.venue_name}</p>
                        {workshop.venue_address && (
                          <p className="text-xs text-neutral-400 mt-0.5">{workshop.venue_address}</p>
                        )}
                      </div>
                    </div>
                    ...
                    {workshop.duration && (
                      <div className="flex items-start gap-4 p-3 rounded-lg hover:bg-neutral-50 transition-colors">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-neutral-500">{workshop.duration}</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center gap-4 text-xs font-medium text-neutral-400 border-t border-neutral-100 pt-4">
                      <span className="flex items-center gap-1"><CalendarIcon className="w-4 h-4" /> {formatDate(r.date).split(",")[0]}</span>
                      {r.duration && <span className="flex items-center gap-1"><ClockIcon className="w-4 h-4" /> {r.duration}</span>}
                    </div>
                  </div>
                </Link>
              ))}
